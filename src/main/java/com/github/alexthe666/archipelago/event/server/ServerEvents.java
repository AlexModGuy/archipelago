...
public class ServerEvents {
    ...
    public void onEntityUpdate(LivingUpdateEvent event) {
        this.handleMaterialAcceleration(event.getEntity().getEntityBoundingBox(), Material.CORAL, event.getEntity());
        ...
        if (event.getEntity() instanceof EntityPlayer) {
            ...
            if (properties.teleportTime > 0 && properties.teleportTime <= 300) {
                ...
                for (int i = 0; i < 5; i++) {
                    ...
                    float extraX = radius * MathHelper.sin((float) (Math.PI + angle));
                    float extraZ = radius * MathHelper.cos(angle);
                    ...
                    Archipelago.PROXY.spawnParticle(TropicParticle.TELEPORT, event.getEntity().worldObj, (float) event.getEntity().posX + extraX, (float) event.getEntity().posY + extraY, (float) event.getEntity().posZ + extraZ, 0, 0, 0);
                }
            }
