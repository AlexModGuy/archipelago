	...
	public void onEntityUpdate(LivingUpdateEvent event) {
		...
		if (event.getEntity() instanceof EntityPlayer) {
			...
			if (properties.teleportTime > 0 && properties.teleportTime <= 300) {
				...
				event.getEntity().worldObj.spawnParticle(EnumParticleTypes.END_ROD, (float) (event.getEntity().posX + (rand.nextDouble() - 0.5D) * event.getEntity().width), (float) (event.getEntity().posY + rand.nextDouble() * event.getEntity().height), (float) (event.getEntity().posZ + (rand.nextDouble() - 0.5D) * event.getEntity().width), 0, 0, 0, new int[0]);
