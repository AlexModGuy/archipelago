    ...
    public void onEntityUpdate(LivingUpdateEvent event) {
        ...
        if (event.getEntity() instanceof EntityPlayer) {
            ...
            if (properties.teleportTime > 0 && properties.teleportTime <= 300) {
                ...
                Archipelago.PROXY.spawnParticle(EnumParticle.TELEPORT, event.getEntity().worldObj, (float) (event.getEntity().posX + (rand.nextDouble() - 0.5D) * event.getEntity().width), (float) (event.getEntity().posY + rand.nextDouble() * event.getEntity().height), (float) (event.getEntity().posZ + (rand.nextDouble() - 0.5D) * event.getEntity().width), 0, 0, 0);
                event.getEntity().worldObj.spawnParticle(EnumParticleTypes.END_ROD, (float) (event.getEntity().posX + (rand.nextDouble() - 0.5D) * event.getEntity().width), (float) (event.getEntity().posY + rand.nextDouble() * event.getEntity().height), (float) (event.getEntity().posZ + (rand.nextDouble() - 0.5D) * event.getEntity().width), 0, 0, 0);
                float sub = (float) (Math.random() * 0.30000001192092896D);
                event.getEntity().worldObj.spawnParticle(EnumParticleTypes.SPELL_MOB, (float) (event.getEntity().posX + (rand.nextDouble() - 0.5D) * event.getEntity().width), (float) (event.getEntity().posY + rand.nextDouble() * event.getEntity().height), (float) (event.getEntity().posZ + (rand.nextDouble() - 0.5D) * event.getEntity().width), 0.95F - sub, 0.8439F - sub, 0.3135F - sub);
