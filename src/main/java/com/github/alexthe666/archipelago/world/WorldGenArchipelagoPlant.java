	...
	public boolean generate(World worldIn, Random rand, BlockPos position){
		...
		if(worldIn != null && plantType != null && plantType.block != null){
			for (int i = 0; i < 64; ++i){
				...
				if (plantType.canSpawnIn(worldIn.getBiomeGenForCoords(blockpos)) && worldIn.isBlockLoaded(blockpos) && (worldIn.getBlockState(blockpos).getBlock() == ModFluids.tropical_water || worldIn.isAirBlock(blockpos)) && (!worldIn.provider.getHasNoSky() || blockpos.getY() < 254) && plantType.block.canPlaceBlockAt(worldIn, blockpos)){
