...
public class GenLayerShallows extends GenLayer {
    public GenLayerShallows(long seed, GenLayer parent) {
        super(seed);
        this.parent = parent;
    }
    ...
    public int[] getInts(int areaX, int areaY, int areaWidth, int areaHeight) {
        ...
        for (int deltaZ = 0; deltaZ < areaHeight; ++deltaZ) {
            for (int deltaX = 0; deltaX < areaWidth; ++deltaX) {
                ...
                this.replace(parentBiomes, newBiomes, deltaX, deltaZ, areaWidth, biomeIndex, Biome.getIdForBiome(ModWorld.tropicShallows));
            }
        }
        return newBiomes;
    }
